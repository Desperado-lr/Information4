...
def send_sms_code():
    ...
    2.校验参数(参数是否有值，判断是否正确)
    3.先从redis中取出真实的内容
    4.与用户的验证码内容进行对比，如果不一致，那么返回验证码输入有误
    5.如果一直，生成验证码的内容(随机数据)
    6.发送短信验证码
    7.告知发送结果
    :return:
    """

    '{"mobiel":"18580838050","image_code":"AAAA","image_code_id":"dawfafawfw"}'
    # params_dict = json.loads(request.data)
    params_dict = request.json

    mobile = params_dict.get("mobiel")
    ...
    # 4.与用户的验证码内容进行对比，如果不一致，那么返回验证码输入有误
    if real_image_code.upper() != image_code:
        return jsonify(errno=RET.DATAERR, errmsg="验证码输入错误")

    #  5.如果一直，生成验证码的内容(随机数据)
    # s随机数字，保证数字长度为6位，不够再前面补0
